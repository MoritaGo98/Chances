...
const fs = require ('fs');/*
//leer un archivo
fs.readFile('hey.txt','utf8',(error,datos)=>{
   if(error){
    console.log('no se puede leer el archivo')
   }else{
    console.log('se puede leer el archivo')
   }
}
);
...
//Escribir un archivo
fs.writeFile('Orale.txt','Este es un nuevo archivo nuevo.',(error) => {
if(error){
    console.log('no se puede leer el archivo');
}else{
    console.log('se puede leer el archivo');
...
fs.access('Orale.js',fs.constants.F_OK,(error)=>{
    if(error){
...
fs.rename('Orale.txt','nuevonombre.txt',(error) =>{
    if(error){
...
//Eliminar un archivo
fs.unlink('hey.txt',(error)=>{
    if(error){
        console.log('No se elimino el archivo')
    }else{
        console.log('Si se elimino el archivo')
    }
}
);
...
fs.readdir('.',(error, archivos)=>{
    if(error){
        console.log('No se pued eleeer la carpeta')
    }else{
        console.log('se pudo leear la carpeta')
    }
}
);
*/
